...
Y0=-350000*sin(45*(pi/180));
Z0=350000*cos(45*(pi/180));
...
cont=1;
for t=0:s:length
    % data used in the angle plots to represent the time in months:
    k(cont)=t/(30*24*3600);
    
    % SUN-PLEX-EARTH (SPE) ANGLE:
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Vectors Plex-Earth and Plex-Sun:
    PE=[(XL-lx(cont)) (0-ly(cont)) (0-lz(cont))];
    PS=[(AU+XL-lx(cont)) (0-ly(cont)) (0-lz(cont))];
    % Calculation of Sun-Plex-Earth angle:
    PEmod=sqrt(((PE(1))^2)+((PE(2))^2)+((PE(3))^2));
    PSmod=sqrt(((PS(1))^2)+((PS(2))^2)+((PS(3))^2));
    SPEang(cont)=(180/pi)*acos((PE(1)*PS(1)+PE(2)*PS(2)+PE(3)*PS(3))/(PEmod*PSmod));
...
% LISSAJOUS ORBIT:
%%%%%%%%%%%%%%%%%%
fig_sim_yz = figure('Name','Lissajous YZ-plane');
plot(ly,lz,ry,rz,'-');
%axis([-100000 100000 -100000 100000])
xlabel('Y (km)');
ylabel('Z (km)');
title('YZ-plane Lissajous orbit');
grid on;
saveas(fig_sim_yz,'img/simulated/trajectory-yz-plane.png');

fig_sim_yx = figure('Name','Lissajous YX-plane');
plot(ly,lx,'-');
%axis([-100000 100000 -100000 100000]);
xlabel('Y (km)');
ylabel('X (km)');
title('YX-plane Lissajous orbit');
grid on;
saveas(fig_sim_yx,'img/simulated/trajectory-yx-plane.png');

fig_sim_xz = figure('Name','Lissajous XZ-plane');
plot(lx,lz,'-');
%axis([-100000 100000 -100000 100000]);
xlabel('X (km)');
ylabel('Z (km)');
title('XZ-plane Lissajous orbit');
grid on;
saveas(fig_sim_xz,'img/simulated/trajectory-xz-plane.png');

fig_sim_3d = figure('Name','Lissajous 3D');
plot3(lx,ly,lz,'-');
...
title('3D PLEX orbit');
grid on;
saveas(fig_sim_3d,'img/simulated/trajectory-3d.png');

fig_sim_speang = figure('Name','Sun-Plex-Earth Angle');
set(fig_sim_speang,'position',[10,10,1200,300])
plot(k,SPEang,'-');
xlabel('Time (months)');
ylabel('Angle (degrees)');
title('Sun-Plex-Earth Angle');
grid on;
saveas(fig_sim_speang,'img/simulated/trajectory-spe-angle.png');
